...
2. Then, set up the application:

`>> pip install -e .`

3. To run the application, you must first set the `FLASK_APP` environment variable to `cryptonalysis`.

In Windows, run the following command:

`>> set FLASK_APP=cryptonalysis`

For *NIX systems, run:

`>> export FLASK_APP=cryptonalysis`
