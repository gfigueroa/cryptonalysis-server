...
def get_data_filename(predictor_class, lookahead_days, starting_date, ending_date, window_size, normalize,
                      normalize_by_row):
    """
    Get the path and filename used for the data file containing the preprocessed data based on the preprocessing
    parameters.
    An example value returned would be '../data/pre_ProbabilityPredictor_2018-01-01-2018-07-10_win30_norm_col'.
    ...
    filename = 'pre_{0}({1})_{2}-{3}_win{4}{5}.csv'.format(predictor_class.__name__, lookahead_days, starting_date,
                                                           ending_date, window_size, norm_string)
    file_path = os.path.join(DATA_FOLDER, filename)
    return file_path


def save_data_file(transactions_df, predictor_class, lookahead_days, starting_date, ending_date, window_size, normalize,
                   normalize_by_row):
    """
    ...
    data_file_path = get_data_filename(predictor_class, lookahead_days, starting_date, ending_date, window_size,
...
def run_data_pipeline(historical_file, starting_date=None, ending_date=None, price_column='Close',
    ...
    ending_date = ending_date or df['Date'].dt.date.iat[-1]
    ...
    # 1. Check if the preprocessed datafile already exists
    lookahead_days = kwargs['lookahead_days'] if 'lookahead_days' in kwargs else CryptoPredictor.LOOKAHEAD_DAYS
    if save_data:
        preprocessed_data_filename = get_data_filename(predictor_class, lookahead_days, starting_date, ending_date,
    ...
    # 3.5. Get any additional predictor parameters
    ...
    # 6. Save data
    if save_data:
        save_data_file(transactions_df, predictor_class, lookahead_days, starting_date, ending_date, window_size,
                       normalize, normalize_by_row)
